...
DATA = OUTPUT_DIR + "new_data_entries.json"
DICT = OUTPUT_DIR + "new_dict.json"
...
if __name__ == '__main__':
    ...
    for y in pool.imap_unordered(findMatchForASingleEntry, data):
        ...
        print(
            '"5ApproxMatchingNewDataPhonetics.py" Running Time: %s, Progress %d/%d; Average Single Match Time %s; '
            'Estimate Time Remaining: '
              '%s.\r' % (p_t, cnt, len(data), itr_time, est_remain_time))
    print("Saving results...")
    with open(OUTPUT_DIR + "soundex_matching_new_data_result.json", 'w') as fp:
